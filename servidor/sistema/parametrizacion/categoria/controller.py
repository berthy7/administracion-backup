...
from servidor.sistema.parametrizacion.categoria.manager import CategoriaManager

import json


class CategoriaController(CrudController):
    manager = CategoriaManager
    html_index = "sistema/parametrizacion/categoria/views/index.html"

    routes = {
        '/categoria': {'GET': 'index', 'POST': 'table'},
        '/categoria_insert': {'POST': 'insert'},
        '/categoria_update': {'PUT': 'edit', 'POST': 'update'},
        '/categoria_state': {'POST': 'state'},
        '/categoria_delete': {'POST': 'delete'},
        '/categoria_list': {'POST': 'data_list'},
    }
    ...
    def insert(self):
        try:
            ...
            CategoriaManager(self.db).insert(objeto)
    ...
    def update(self):
        try:
            ...
            diccionary['ip'] = self.request.remote_ip
            objeto = self.manager(self.db).entity(**diccionary)
            CategoriaManager(self.db).update(objeto)
    ...
    def state(self):
        try:
            self.set_session()
            diccionary = json.loads(self.get_argument("object"))
            result = CategoriaManager(self.db).state(diccionary['id'], diccionary['estado'], self.get_user_id(), self.request.remote_ip)
