...
from servidor.sistema.parametrizacion.parentesco.manager import ParentescoManager

import json


class ParentescoController(CrudController):
    manager = ParentescoManager
    html_index = "sistema/parametrizacion/parentesco/views/index.html"

    routes = {
        '/parentesco': {'GET': 'index', 'POST': 'table'},
        '/parentesco_insert': {'POST': 'insert'},
        '/parentesco_update': {'PUT': 'edit', 'POST': 'update'},
        '/parentesco_state': {'POST': 'state'},
        '/parentesco_delete': {'POST': 'delete'},
        '/parentesco_list': {'POST': 'data_list'},
    }
    ...
    def insert(self):
        try:
            ...
            ParentescoManager(self.db).insert(objeto)
    ...
    def update(self):
        try:
            ...
            diccionary['ip'] = self.request.remote_ip
            objeto = self.manager(self.db).entity(**diccionary)
            ParentescoManager(self.db).update(objeto)
    ...
    def state(self):
        try:
            self.set_session()
            diccionary = json.loads(self.get_argument("object"))
            result = ParentescoManager(self.db).state(diccionary['id'], diccionary['estado'], self.get_user_id(), self.request.remote_ip)
