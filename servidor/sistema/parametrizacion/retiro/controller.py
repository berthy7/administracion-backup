...
from servidor.sistema.parametrizacion.retiro.manager import RetiroManager

import json


class RetiroController(CrudController):
    manager = RetiroManager
    html_index = "sistema/parametrizacion/retiro/views/index.html"

    routes = {
        '/retiro': {'GET': 'index', 'POST': 'table'},
        '/retiro_insert': {'POST': 'insert'},
        '/retiro_update': {'PUT': 'edit', 'POST': 'update'},
        '/retiro_state': {'POST': 'state'},
        '/retiro_delete': {'POST': 'delete'},
        '/retiro_list': {'POST': 'data_list'},
    }
    ...
    def insert(self):
        try:
            ...
            RetiroManager(self.db).insert(objeto)
    ...
    def update(self):
        try:
            ...
            diccionary['ip'] = self.request.remote_ip
            objeto = self.manager(self.db).entity(**diccionary)
            RetiroManager(self.db).update(objeto)
    ...
    def state(self):
        try:
            self.set_session()
            diccionary = json.loads(self.get_argument("object"))
            result = RetiroManager(self.db).state(diccionary['id'], diccionary['estado'], self.get_user_id(), self.request.remote_ip)
