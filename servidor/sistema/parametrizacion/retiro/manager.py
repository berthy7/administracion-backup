...
class RetiroManager(SuperManager):

    def __init__(self, db):
        super().__init__(Retiro, db)
    ...
    def all_data(self, idu):
        privilegios = UsuarioManager(self.db).get_privileges(idu, '/retiro')
        datos = self.db.query(self.entity).filter(self.entity.enabled).all()
        list = []

        for item in datos:
            is_active = item.estado
            disable = 'disabled' if 'retiro_update' not in privilegios else ''
            estado = 'Activo' if is_active else 'Inactivo'
            check = 'checked' if is_active else ''
            delete = 'retiro_delete' in privilegios
    ...
    def insert(self, objeto):
        ...
        b = Bitacora(fkusuario=objeto.user, ip=objeto.ip, accion="Registró retiro.", fecha=fecha, tabla="parametrizacion_retiro", identificador=a.id)
        super().insert(b)
        return a
    ...
    def update(self, objeto):
        ...
        b = Bitacora(fkusuario=objeto.user, ip=objeto.ip, accion="Modificó retiro.", fecha=fecha, tabla="parametrizacion_retiro", identificador=a.id)
        super().insert(b)
        return a

    def state(self, id, estado, user, ip):
        x = self.db.query(self.entity).filter(self.entity.id == id).one()
        mensaje = "Habilitó retiro" if estado else "Deshabilitó retiro"
        x.estado = estado

        fecha = BitacoraManager(self.db).fecha_actual()
        b = Bitacora(fkusuario=user, ip=ip, accion=mensaje, fecha=fecha, tabla="parametrizacion_retiro", identificador=id)
        super().insert(b)
        self.db.merge(x)
        self.db.commit()
        return x
    ...
    def delete(self, id, user, ip):
        ...
        fecha = BitacoraManager(self.db).fecha_actual()
        b = Bitacora(fkusuario=user, ip=ip, accion="Eliminó retiro", fecha=fecha, tabla="parametrizacion_retiro", identificador=id)
        super().insert(b)
        self.db.merge(x)
        self.db.commit()
