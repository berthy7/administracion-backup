...
class ExpedidoManager(SuperManager):

    def __init__(self, db):
        super().__init__(Expedido, db)
    ...
    def all_data(self, idu):
        privilegios = UsuarioManager(self.db).get_privileges(idu, '/expedido')
        datos = self.db.query(self.entity).filter(self.entity.enabled).all()
        list = []

        for item in datos:
            is_active = item.estado
            disable = 'disabled' if 'expedido_update' not in privilegios else ''
            estado = 'Activo' if is_active else 'Inactivo'
            check = 'checked' if is_active else ''
            delete = 'expedido_delete' in privilegios
    ...
    def insert(self, objeto):
        ...
        b = Bitacora(fkusuario=objeto.user, ip=objeto.ip, accion="Registró expedido.", fecha=fecha, tabla="parametrizacion_expedido", identificador=a.id)
        super().insert(b)
        return a
    ...
    def update(self, objeto):
        ...
        b = Bitacora(fkusuario=objeto.user, ip=objeto.ip, accion="Modificó expedido.", fecha=fecha, tabla="parametrizacion_expedido", identificador=a.id)
        super().insert(b)
        return a

    def state(self, id, estado, user, ip):
        x = self.db.query(self.entity).filter(self.entity.id == id).one()
        mensaje = "Habilitó expedido" if estado else "Deshabilitó expedido"
        x.estado = estado

        fecha = BitacoraManager(self.db).fecha_actual()
        b = Bitacora(fkusuario=user, ip=ip, accion=mensaje, fecha=fecha, tabla="parametrizacion_expedido", identificador=id)
        super().insert(b)
        self.db.merge(x)
        self.db.commit()
        return x
    ...
    def delete(self, id, user, ip):
        ...
        fecha = BitacoraManager(self.db).fecha_actual()
        b = Bitacora(fkusuario=user, ip=ip, accion="Eliminó expedido", fecha=fecha, tabla="parametrizacion_expedido", identificador=id)
        super().insert(b)
        self.db.merge(x)
        self.db.commit()
