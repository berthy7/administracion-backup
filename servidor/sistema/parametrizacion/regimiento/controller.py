...
from servidor.sistema.parametrizacion.regimiento.manager import RegimientoManager

import json


class RegimientoController(CrudController):
    manager = RegimientoManager
    html_index = "sistema/parametrizacion/regimiento/views/index.html"

    routes = {
        '/regimiento': {'GET': 'index', 'POST': 'table'},
        '/regimiento_insert': {'POST': 'insert'},
        '/regimiento_update': {'PUT': 'edit', 'POST': 'update'},
        '/regimiento_state': {'POST': 'state'},
        '/regimiento_delete': {'POST': 'delete'},
        '/regimiento_list': {'POST': 'data_list'},
    }
    ...
    def insert(self):
        try:
            ...
            RegimientoManager(self.db).insert(objeto)
    ...
    def update(self):
        try:
            ...
            diccionary['ip'] = self.request.remote_ip
            objeto = self.manager(self.db).entity(**diccionary)
            RegimientoManager(self.db).update(objeto)
    ...
    def state(self):
        try:
            self.set_session()
            diccionary = json.loads(self.get_argument("object"))
            result = RegimientoManager(self.db).state(diccionary['id'], diccionary['estado'], self.get_user_id(), self.request.remote_ip)
