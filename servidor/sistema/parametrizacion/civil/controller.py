...
from servidor.sistema.parametrizacion.civil.manager import CivilManager

import json


class CivilController(CrudController):
    manager = CivilManager
    html_index = "sistema/parametrizacion/civil/views/index.html"

    routes = {
        '/civil': {'GET': 'index', 'POST': 'table'},
        '/civil_insert': {'POST': 'insert'},
        '/civil_update': {'PUT': 'edit', 'POST': 'update'},
        '/civil_state': {'POST': 'state'},
        '/civil_delete': {'POST': 'delete'},
        '/civil_list': {'POST': 'data_list'},
    }
    ...
    def insert(self):
        try:
            ...
            CivilManager(self.db).insert(objeto)
    ...
    def update(self):
        try:
            ...
            diccionary['ip'] = self.request.remote_ip
            objeto = self.manager(self.db).entity(**diccionary)
            CivilManager(self.db).update(objeto)
    ...
    def state(self):
        try:
            self.set_session()
            diccionary = json.loads(self.get_argument("object"))
            result = CivilManager(self.db).state(diccionary['id'], diccionary['estado'], self.get_user_id(), self.request.remote_ip)
