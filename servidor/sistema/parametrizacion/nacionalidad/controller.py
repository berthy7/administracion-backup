...
from servidor.sistema.parametrizacion.nacionalidad.manager import NacionalidadManager

import json


class NacionalidadController(CrudController):
    manager = NacionalidadManager
    html_index = "sistema/parametrizacion/nacionalidad/views/index.html"

    routes = {
        '/nacionalidad': {'GET': 'index', 'POST': 'table'},
        '/nacionalidad_insert': {'POST': 'insert'},
        '/nacionalidad_update': {'PUT': 'edit', 'POST': 'update'},
        '/nacionalidad_state': {'POST': 'state'},
        '/nacionalidad_delete': {'POST': 'delete'},
        '/nacionalidad_list': {'POST': 'data_list'},
    }
    ...
    def insert(self):
        try:
            ...
            NacionalidadManager(self.db).insert(objeto)
    ...
    def update(self):
        try:
            ...
            diccionary['ip'] = self.request.remote_ip
            objeto = self.manager(self.db).entity(**diccionary)
            NacionalidadManager(self.db).update(objeto)
    ...
    def state(self):
        try:
            self.set_session()
            diccionary = json.loads(self.get_argument("object"))
            result = NacionalidadManager(self.db).state(diccionary['id'], diccionary['estado'], self.get_user_id(), self.request.remote_ip)
