...
function load_table(data_tb) {
    var tabla = $(id_table).DataTable({
        ...
        "order": [ [0, 'desc'] ],
        columnDefs: [ { width: '5%', targets: [0] }, { width: '20%', targets: [1, 2, 3] } ],
...
function edit_item(e) {
    ...
    ajax_call_get('descuento_update',{
        _xsrf: getCookie("_xsrf"),
        object: obj
    },function(response){
...
function set_enable(e) {
    ...
    if (!b) {
        cb_title = "¿Está seguro de que desea dar de baja la descuento?"
        cb_text = ""
        cb_type = "warning"
    } else {
        cb_title ="¿Está seguro de que desea dar de alta la descuento?"
        cb_text = ""
        cb_type = "info"
    }
...
function delete_item(e) {
    Swal.fire({
        icon: "warning",
        title: "¿Está seguro de que desea eliminar permanentemente la descuento?",
        text: "",
        showCancelButton: true,
        allowOutsideClick: false,
        confirmButtonColor: '#1565c0',
