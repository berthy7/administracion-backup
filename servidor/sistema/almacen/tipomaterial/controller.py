...
class TipoMaterialController(CrudController):
    manager = TipoMaterialManager
    html_index = "sistema/almacen/tipomaterial/views/index.html"

    routes = {
        '/tipomaterial': {'GET': 'index', 'POST': 'table'},
        '/tipomaterial_insert': {'POST': 'insert'},
        '/tipomaterial_update': {'PUT': 'edit', 'POST': 'update'},
        '/tipomaterial_state': {'POST': 'state'},
        '/tipomaterial_delete': {'POST': 'delete'},
        '/tipomaterial_list': {'POST': 'data_list'},
    }
    ...
    def insert(self):
        try:
            ...
            TipoMaterialManager(self.db).insert(objeto)
    ...
    def update(self):
        try:
            ...
            diccionary['ip'] = self.request.remote_ip
            objeto = self.manager(self.db).entity(**diccionary)
            TipoMaterialManager(self.db).update(objeto)
    ...
    def state(self):
        try:
            self.set_session()
            diccionary = json.loads(self.get_argument("object"))
            result = TipoMaterialManager(self.db).state(diccionary['id'], diccionary['estado'], self.get_user_id(), self.request.remote_ip)
