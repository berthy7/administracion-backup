...
from servidor.sistema.usuarios.ajuste.manager import AjusteManager
...
class AjusteController(CrudController):
    ...
    def insert(self):
        try:
            ...
            AjusteManager(self.db).insert(objeto)
    ...
    def state(self):
        try:
            self.set_session()
            diccionary = json.loads(self.get_argument("object"))
            result = AjusteManager(self.db).state(diccionary['id'], diccionary['estado'], self.get_user_id(), self.request.remote_ip)
